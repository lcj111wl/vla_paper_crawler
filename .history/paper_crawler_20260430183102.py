...
def _derive_pdf_link(paper: Dict[str, Any]) -> Optional[str]:
    ...
    if isinstance(doi, str) and doi.lower().startswith('arxiv:'):
...
def patch_missing_fields(notion_client: "NotionClient",
    ...
    for idx, paper in enumerate(papers_to_process):
        try:
            ...
            # 防御性 title 处理
            safe_title = (paper.get('title') or 'Unknown')[:40] if isinstance(paper, dict) else 'Unknown'
            ...
            if field_type == 'pdf_url':
                ...
                if pdf_url:
                    ...
                    logger.info(f"✅ 生成 PDF Link: {safe_title} → {pdf_url[:60]}")
            ...
            # 优先级2: Citations（Semantic Scholar API）
                ...
                if cites is not None:
                    ...
                    logger.info(f"✅ 添加引用数: {safe_title} → {cites} citations")
            ...
            # 优先级2+: Institutions（Semantic Scholar API）
                ...
                if institutions:
                    ...
                    logger.info(f"✅ 添加机构: {safe_title} → {len(institutions)} 个机构")
            ...
            # 优先级3: Recommend Score（LLM）
                ...
                if score is not None:
                    ...
                    logger.info(f"✅ LLM 评分: {safe_title} → {score}")
            ...
            # 防御性处理：paper.get('title') 可能为 None，避免对 None 做切片
            raw_title = paper.get('title') if isinstance(paper, dict) else None
            safe_title = (raw_title or 'Unknown')
            logger.error(f"补全字段失败 ({field_type}): {safe_title[:40]} - {e}")
            failed += 1
...
class NotionClient:
    ...
    def __init__(self, token: str, database_id: str):
        ...
        self._title_prop_name: Optional[str] = None

    def _get_title_property_name(self) -> str:
        """获取数据库中 title 类型属性的真实名称（可能为空字符串或其它名称）"""
        if self._title_prop_name is not None:
            return self._title_prop_name
        try:
            props = self._get_database()
            for k, v in props.items():
                if v.get('type') == 'title':
                    self._title_prop_name = k
                    return k
        except Exception:
            pass
    ...
    def check_duplicate(self, title: Optional[str] = None, doi: Optional[str] = None, url: Optional[str] = None) -> bool:
        ...
        if title:
            filters.append({
                "property": self._get_title_property_name(),
                "title": {"equals": title}
            })
        if doi:
            filters.append({
    ...
    def fetch_existing_papers(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        field_mapping = {
            self._get_title_property_name(): 'title',
            'userDefined:URL': 'url',
            'PDF Link': 'pdf_url',
            'DOI': 'doi',
            'Year': 'year',
